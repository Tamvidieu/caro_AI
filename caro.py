...
class Game:
    ...
    def set_piece(self, row, col):
        if self.board.matrix[row][col] == 0:
            self.board.matrix[row][col] = 2 if self.black_turn else 1
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, (222, 222, 222), [self.start_x - self.edge_size, self.start_y - self.edge_size, (self.board.size - 1)
                                                  * self.grid_size + self.edge_size * 2, (self.board.size - 1) * self.grid_size + self.edge_size * 2], 0)

        for r in range(self.board.size+1):
            y = self.start_y + r * self.grid_size - self.grid_size/2
            pygame.draw.line(screen, (0, 0, 0), [self.start_x-self.grid_size/2, y], [
                             self.start_x + self.grid_size * (self.board.size - 1) + self.grid_size/2, y], 2)

        for c in range(self.board.size+1):
            x = self.start_x + c * self.grid_size - self.grid_size/2
            pygame.draw.line(screen, (0, 0, 0), [x, self.start_y-self.grid_size/2], [
                             x, self.start_y + self.grid_size * (self.board.size - 1) + self.grid_size/2], 2)
            ...
            for c in range(self.board.size):
                ...
                if piece != 0:
                    if piece == 1:
                        ...
                        pygame.draw.line(screen, color, [x - half_size + 5, y - half_size  + 3 ], [x + half_size - 5, y + half_size - 3], 7)
                        pygame.draw.line(screen, color, [x + half_size  - 5, y - half_size  + 3], [x - half_size + 5, y + half_size - 3], 7)
                    else:
                        color = (218,31,37)
                        radius = self.grid_size / 5 * 2
                        pygame.draw.circle(screen, color, [x, y], radius, width=7)
...
class CaroUI():
    ...
    def __init__(self, name="Caro", size=12, mode=1):
        pygame.init()
        pygame.display.set_caption(name)
        self.width = 30 * size + 30
        self.height = 30 * size + 100
    ...
    def draw(self):
        ...
        if self.game.game_over:
            self.screen.blit(self.font.render("{0} Win !!!".format("Ban" if self.game.winner == 2 else "May"), True, (0, 0, 0)), (self.width / 3, self.height - 50))
        pygame.display.update()


if __name__ == '__main__':
    game = CaroUI("Caro", 12, CaroUI.PVC)
    game.loop()
